...
class AgeCalculator(QWidget):
    ...
    def load_data(self):
        ...
        # result = cursor.execute("SELECT * FROM User_age")
        # print(result)
        # self.table_widget.setRowCount(len(result))
        # for row, user in enumerate(result):
        #     self.table_widget.setItem(row, 0, QTableWidgetItem(user[0]))  # Name
        #     self.table_widget.setItem(row, 1, QTableWidgetItem(user[1]))  # Date of Birth
